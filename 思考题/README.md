...
1. （p24页）考虑何时发生这几种情况：1.两数相加，有符号数溢出，但无符号数不溢出；2.有符号数溢出，无符号数不溢出；3.同时发生有符号数与无符号数溢出。
...
2. （p41页）怎样调用print_func()，使得它打印自身的指令？怎么写出一个打印自身整个程序文件的程序？

   > - `print_func()`的核心功能是通过函数指针遍历指令的二进制内容（按 RISC-V 架构的 4 字节指令格式），直到遇到`ret`指令（机器码`0x00008067`）为止。要让它打印自身的指令，只需将`print_func`函数自身的地址作为参数传入即可。
   > - 这类程序称为 “自打印程序”（quine），其核心是**程序运行时输出的内容与自身源代码完全一致**。实现的关键是 “自引用”：程序需包含自身代码的表示（如字符串），并通过格式化输出将其完整还原。、
   >
   > 这两种场景均体现了 “程序即数据” 的核心思想 —— 代码在内存中可被视为可操作的数据，通过指针或字符串等形式实现自引用和自处理。